 ...
 *
 * 一个使用有限空间的文件系统的缓存。每个缓存实例都有一个类型为string的key和固定长度（fixed number）的值
 * 缓存的key必须为a-z0-0和下划线，value为byte数组（可以进入一个流或者文件）
 *
 ...
 *
 * 缓存文件必须只能用来存贮缓存，缓存会因为目录被删除而删除，假若多线程访问同一个文件会抛出异常
 *
...
public final class DiskLruCache implements Closeable, Flushable {
  ...
  private static final String READ = "READ";
    ...
    /*
     * This cache uses a journal file（日志文件） named "journal". A typical journal file
     * looks like this:
  ...
  private final int valueCount;// 一个键对应几个文件
  private long size = 0;
  private BufferedSink journalWriter;
  private final LinkedHashMap<String, Entry> lruEntries = new LinkedHashMap<>(0, 0.75f, true);
  private int redundantOpCount; //这个变量记录的应该是记录文件的操作数把，防止日志文件过大。。
  ...
  /** Used to run 'cleanupRunnable' for journal rebuilds.用于运行日志重构 */
  ...
  private void readJournalLine(String line) throws IOException {
    //每一行的数据至少包含一个空格，clean 至少有两个，其他只有一个，用这个可以识别每一行数据的种类
    int firstSpace = line.indexOf(' ');//第一个空格的下标
    if (firstSpace == -1) {
      throw new IOException("unexpected journal line: " + line);
    }

    int keyBegin = firstSpace + 1;
    int secondSpace = line.indexOf(' ', keyBegin);//第二个空格的下标
    final String key;
    if (secondSpace == -1) {
    ...
    //如果是clean，那么把readable设置为true同时把editor（对应的inputStream是指为null）
    if (secondSpace != -1 && firstSpace == CLEAN.length() && line.startsWith(CLEAN)) {
      String[] parts = line.substring(secondSpace + 1).split(" ");
      entry.readable = true;
      entry.currentEditor = null;
      entry.setLengths(parts);//一对多记录设置长度
    } else if (secondSpace == -1 && firstSpace == DIRTY.length() && line.startsWith(DIRTY)) {
      //数据为dirty就是说editor已经交付给了用户，这个时候editor不为null
   ...
   * 统计缓存文件的大小
   * 由于上个方法把dirty类型的行都统计了进去，但是每个dirty后面都应该跟着改dirty的key对应的clean行，否则这一行dirty是无效的
   * 所以，在这里删除了只有单独行dirty的文件以及在缓存中的数据
   * 执行完这一步后，内存的缓存中就只有类型为clean的记录了。
   */
  private void processJournal() throws IOException {
    ...
    for (Iterator<Entry> i = lruEntries.values().iterator(); i.hasNext(); ) {
      ...
      if (entry.currentEditor == null) {
        ...
        //editor!=null说明数据为dirty，但是来到这一步，数据不应该为dirty，所以删除
        entry.currentEditor = null;
        for (int t = 0; t < valueCount; t++) {
   ...
   * exists.
   * 创建一个新的日志文件，如果当前的日志文件存在，那么它将会被替换
   */
  private synchronized void rebuildJournal() throws IOException {
    ...
    try {
      ...
      for (Entry entry : lruEntries.values()) {
        if (entry.currentEditor != null) {
          //从这里看出来了，如果在lru中的实例的editor不为null，那么该行数据就是dirty的
   ...
   * 返回一个快照，如果它不存在或者不可读，那么返回null
   * 如果返回的一个快照，那么它将会移到队列的头
   */
  public synchronized Snapshot get(String key) throws IOException {
    ...
    //因为是linkedHadhMap，所以对它的访问会把数据移到队列的头
    Entry entry = lruEntries.get(key);
  ...
  private synchronized void completeEdit(Editor editor, boolean success) throws IOException {
    ...
    //成功则把dirty文件重命名为clean文件，并重新计算缓存大小，否则删除dirty文件
    for (int i = 0; i < valueCount; i++) {
    ...
    redundantOpCount++;
    //因为已经提交了，所以不再编辑状态了，所以editor设置为null了
    entry.currentEditor = null;

    //成功后还要往日志文件写入一个clean的日志，否则，写入一条remove数据
    if (entry.readable | success) {
      entry.readable = true;
      journalWriter.writeUtf8(CLEAN).writeByte(' ');
      journalWriter.writeUtf8(entry.key);
      entry.writeLengths(journalWriter); //clean文件后面对应的是文件的大小啊。。。。
  ...
  public final class Editor {
     ...
     *
     * 返回一个对应 下标的 无缓存输出流，因为我们通常使用都是一个key对应一个缓存，所以一般都是传0，
     * 如果输出流在写的过程中出现错误，那么在commit操作的时候将会终止写文件，因为okhttp使用的是okio库
     * 所以这个输出流逝不会抛出io异常的。
     *
     * 因为是无缓存的，所以还要配合bufferOutPutString使用？
     *
     */
