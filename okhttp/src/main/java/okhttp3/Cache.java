 ...
 *
 * 有时一个请求将导致一个条件缓存命中。如果缓存里面有一个旧的缓存，那么客户端将发布一个询问（conditional），
 * 那么服务端将会发送更新后的响应，或者当客户端的缓存依然有效的时候发送一个“木有改变”的响应
 *
...
public final class Cache implements Closeable, Flushable {
  ...
  private final DiskLruCache cache; //缓存实现，使用lru算法。
